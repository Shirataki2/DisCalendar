...
const SHOWCASES: Showcase[] = [
  {
    ...
    image: {
      src: dialogShot,
      alt: "予定の編集ダイアログ。繰り返し設定、タイトル・日時・色・通知・メンション先・説明の入力欄がある",
    },
  },
  {
    icon: <ShieldCheckIcon className="size-6" aria-hidden />,
    title: "編集できる人を制限 (restricted モード)",
    body: "サーバー設定で restricted モードを有効にすると、「管理者」「サーバー管理」「ロールの管理」「メッセージの管理」のいずれかの権限、または編集を許可されたロールを持つメンバーが予定を追加・編集・削除できます。閲覧はメンバー全員ができます。",
    image: {
      src: settingsShot,
      alt: "サーバー設定ダイアログ。予定の編集を管理権限または指定ロールのあるメンバーに限定するチェックボックスと編集ロールの一覧、補足の ⓘ アイコン、通知先チャンネル・開始時刻の通知・既定の事前通知の設定、外部カレンダーで購読するためのフィード URL",
    },
  },
];
...
const FEATURES: Feature[] = [
  {
    ...
    body: "予定ごとに「30 分前」「1 日前」のような事前通知を最大 10 件まで設定できます。時刻になると Bot がサーバー設定で選んだチャンネルに埋め込みメッセージを投稿するので、リマインドの手間がありません。",
  },
  {
...
const STEPS = [
  ...
  {
    title: "サーバー設定で通知先を決める",
    body: "Web にログインし、カレンダー右上のサーバー設定で通知先チャンネルを選びます。サーバーのオーナーか「管理者」「サーバー管理」「メッセージの管理」「ロールの管理」のいずれかの権限を持つ人が設定でき、Discord の /init でも変更できます。",
  },
  {
    title: "予定を追加する",
