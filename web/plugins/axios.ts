...
export default ({ $axios, req }: any): void => {
  ...
  $axios.onResponse((response: any): void => {
    ...
    if (token) {
      if (token.access_token) { Cookies.set('access_token', token.access_token, { sameSite: 'Lax', secure: false }) }
      if (token.refresh_token) { Cookies.set('refresh_token', token.refresh_token, { sameSite: 'Lax', secure: false }) }
    }
  })
}
