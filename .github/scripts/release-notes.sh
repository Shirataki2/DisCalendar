...
#   .github/scripts/release-notes.sh [タグ] [changelog のパス] [個別ファイルのディレクトリ]
...
# changelog.d/*.md と先頭の未リリース分 (最初の ## より上のエントリ) を出す。
set -euo pipefail
...
if [ -n "$tag" ] && [[ "$tag" != *-* ]]; then
  ...
  load_changelog_fragments "$fragments_dir"
  print_changelog_fragments > "$tmp/section.md"
