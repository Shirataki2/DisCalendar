...
Bot は Better Auth のテーブルを読まず、API がアカウントと Discord ID を対応付けて、送信直前にキャッシュを使わず所属を確認する。
...
API は15秒ごとに最大100件を配信し、失敗時は1分以上あけて最大5回試す。404 / 410 は端末の購読を削除し、5回連続の送信失敗はその端末を停止する。
