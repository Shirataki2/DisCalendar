...
Bot は Better Auth のテーブルを読まず、API がアカウントと Discord ID を対応付ける。配信候補はサーバー・利用者ごとに60秒の既存キャッシュで所属を確認し、非参加者の端末を除く。所属確認の一時エラーは端末ごとの配信待ちで再試行する。送信直前にはキャッシュを使わず所属を再確認する。
