...
- **タグは 1 本ずつ打つ**。前のリリースの Release ワークフローが終わってから次を打つこと。短い間に 2 本以上のタグを
  push すると、後から走った方の「前回リリース」が前の版を拾えず (まだ Release が公開されていないため)、
  リリースノートにその版の分が重複して載ることがある。イメージのタグと `latest` は並走しても壊れないが、ノートの内容は公開順に依存する
