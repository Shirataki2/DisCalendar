...
判断材料は `.github/scripts/release-notes.sh` が出す未リリース分 (`changelog.d/*.md` と、移行前の `web/src/content/changelog.mdx` の未リリース記事)。
これがリリースノートに載る内容そのもの。
...
- スクリプトが 4 か所の書き換えに加え、`changelog.d/*.md` をファイル名順で `web/src/content/changelog.mdx` に集約し、
  「`## vX.Y.Z (YYYY年M月D日)`」の見出しを付ける。取り込んだ個別ファイルの削除もPRに含める。
  移行前の未リリース記事も同じ節に含める。プレリリースでは集約・削除せず、ノート抽出時に個別ファイルを読む
- 見出しの日付 = 利用者に見えるリリース日。既定は実行日なので、タグを打つのが別の日になるなら
  `bump-version.sh 3.1.0 2026-08-30` のように予定日を渡す。レビューが日をまたいでずれたら、
  マージ前に見出しの日付を直す (タグの日とずれていると release.yml が警告を出す)
- この PR には**リリース準備 (バージョン 4 か所と更新履歴の集約・個別ファイルの削除) 以外の変更を混ぜない**。CI が通ったらマージする (squash)
...
- **リリース準備 PR のマージからタグまでの間に機能 PR をマージしない**。その個別ファイルは未集約のまま残り、今回のリリースノートにも更新履歴の今回の節にも入らない (次のリリース扱いになる)
