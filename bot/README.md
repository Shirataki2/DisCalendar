...
| `/create` | 予定の作成。名称 / 開始・終了の年月日時分 (必須)、説明 / 終日 / 色 / 事前通知 4 つまで (任意)。`events` に api と同じ形式で保存するので web のカレンダーにそのまま出る | restricted モードのサーバーでは管理権限 (下記) または編集ロールが必要 |
| `/quick` | 名称・日付・時刻と任意の所要時間（分）を入力し、本人の確認後に予定を保存 | `/create` と同じ |
| `/list [範囲]` | 予定の一覧 (`過去` / `未来` (既定) / `全て` / `今日` / `今週` / `次の予定`)。4 件ずつ「前へ」「次へ」でページ送り、「完了」で消える。web で作った予定も出る | – |
...
| `/settings` | 保存済みの通知先・開始時刻の通知有無・既定の事前通知と、Bot の現在の投稿権限を診断。本人だけに表示 | 管理権限が必要 |
...
`/settings` は設定・権限を変更せず、テスト通知も投稿しません。通知先未設定、削除の可能性（Discord の Unknown Channel）、
参照不能、情報取得失敗を区別し、再設定や権限変更を案内します。Discord から現在のチャンネル・ロール・Bot メンバーを取得し、
`/init` と同じ「チャンネルを見る」「メッセージを送信」「埋め込みリンク」を実効権限で確認します。
スレッドは親の権限上書きと「スレッドでメッセージを送信」で判定します。
権限が揃っていても配信全体の正常性は保証しません。予定ごとの通知、Bot の稼働、スレッドの参加・アーカイブ状態などは別途確認が必要です。
既定の事前通知は Web の新規予定の初期値で、既存の予定や `/create`・`/quick` には自動適用されません。
DB にサーバー設定の行がなければ、サービスの既定値（開始時刻の通知は有効、事前通知は 1 日前・1 時間前）を表示します。
...
`/quick name:定例 date:明日 time:21:00` で簡単に予定を作成できます。
日付は JST の「今日」「明日」「YYYY-MM-DD」（1970〜2099 年）、時刻は「HH:mm」（00:00〜23:59）です。
任意の `duration` は 1〜10080 分（7 日）で、省略時は 60 分。日付をまたぐ終了日時も確認画面に表示します。
実行した本人が 2 分以内に「作成する」を押すと保存し、キャンセル・時間切れでは保存しません。
名称は 32 文字まで、色は青、事前通知なし（開始時の通知はサーバー設定に従う）で、既存の `/create` と同じです。
保存時に restricted モード・管理権限・編集ロールを確認します。保存した予定は Web にも表示されます。
従来の `/create` では説明・終日・色・事前通知も指定できます。
...
| `/create` の検証 | 年月日の範囲チェックのみ | 名称 32 文字 / 説明 1000 文字 (api と同じ上限)、存在しない日付、終了 < 開始、同じ通知の重複を弾く。restricted 判定は api と同じ 4 権限または指定された編集ロール |
...
3. `/init` → `/create` → `/list` の順に試す。`/create` で作った予定は web のカレンダーに表示され、
   ...
   `/quick name:定例 date:明日 time:23:30` で翌日にまたがる終了日時を確認して保存し、Web に表示されることを確認する。
   キャンセル・2 分の時間切れでは保存されないこと、restricted モードで一般メンバーは拒否され編集ロールなら保存できることも試す。
   `/list` の今日・今週・次の予定を選び、終日予定・日付またぎ・同時刻の複数予定を確認する。
...
5. `/settings`: 管理権限のないメンバーは拒否され、管理権限があれば結果が本人だけに表示されることを確認する。
   通知先未設定、設定済み、Bot の送信・埋め込み権限を外した場合、通知先削除後を試す。
   Web で開始時刻の通知・既定の事前通知を変更した値が反映され、診断による設定変更やチャンネル投稿がないことを確認する。
