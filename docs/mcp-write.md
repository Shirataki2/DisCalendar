...
認証設定は [mcp-auth.md](mcp-auth.md)、日時・読み取りの契約は
...
実Codexからの認証・実Discordへの通常反映は依頼者が確認済み。
障害・応答喪失・期限経過後の実機検証は #228、異なる入力の同一キー再送等は #229 へ移管する。

繰り返し予定の既存ツールによる編集・削除は「この回のみ」。例外を記録し、補充で元に戻らない。シリーズの条件変更はWebで行う。読み取り・更新結果には `recurrence`（シリーズID・元の開始日時・条件・バージョン）を追加する。詳細は [繰り返し予定](recurring-events.md) を参照。
