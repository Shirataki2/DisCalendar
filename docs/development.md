...
専用DBでのみ有効化し、OAuthのDDLは通常の起動時マイグレーションでは適用しない。
予定書き込みの操作記録はAPIの新規migrationで用意する（[書き込み契約](mcp-write.md)）。

予定の添付ファイルの設定・API・テスト・回収手順は[添付ファイルの運用](attachments.md)を参照。
