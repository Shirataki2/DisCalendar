...
ステージングのMCPは利用可能で、依頼者が下記8項目を検証済み・期待値どおりと確認した。
検証対象commit: `7c89c4b93b8bcbf6ffffcd3ac97547e60211ab8b`。検証日時: 2026-09-14 22:00 JST。
Codexデスクトップ26.908.40834でも同じ8項目を確認済み。

個人開発の運用として、Git・PRには検証済みか、期待値どおりかの結果のみを記録する。
対象の版・commit・検証日時は併記するが、ログ・スクリーンショット・通信記録などの証跡や保存先はGit・PRに残さない。
認証情報・認可URLのquery・予定本文は記録しない。未報告の項目は未検証のまま残す。

| 対象 | 版・方式 | 確認済みの範囲 | 未検証 |
| --- | --- | --- | --- |
| Codex CLI | 0.154.0、CIMD | 本作業でversionとlogin help確認。#217で登録・認可画面への302 | 実Discordログイン以降の全経路、交渉した仕様版 |
| Codexデスクトップ | 26.908.40834、登録方式の実機確認結果は未記録 | 下記8項目を実機で検証済み・期待値どおり | metadata・PKCE等の個別確認、交渉した仕様版、下記の残項目 |
| SDK | @modelcontextprotocol/server 2.0.0 | 既存DB結合テストで2025-11-25のinitialize、tools/list、connection_info | 実Codexとの仕様交渉 |
| OAuth | Better Auth/MCP/CIMD 1.7.4、CIMD profile mcp-2026-07-28、DCR無効 | モックDB結合テストに加え、実機で15分経過後の継続利用・解除後の操作拒否を確認 | 実配信経路の個別検証、解除後の更新拒否など下記の残項目 |
...
次の表をCLIとデスクトップそれぞれで実施し、検証済み・期待値どおりかを記入する。
今回の8項目についてCLIの検証版・結果との対応は未報告のため、CLI固有の結果は未検証のままとする。
未実施は空欄でなく「未検証」とする。実予定を使わず、専用の検証サーバーと予定で行う。

| 確認済みの手順 | 期待する結果 | 結果（依頼者確認、デスクトップ26.908.40834でも確認済み） |
| --- | --- | --- |
| 検証予定を作成 | Webに1件だけ表示され、ID・URLが返る | 検証済み・期待値どおり |
| タイトルだけ変更 | 説明・通知などが保持される | 検証済み・期待値どおり |
| Webから変更後、古いversionで更新 | 競合として拒否される | 検証済み・期待値どおり |
| 同じキー・同じ入力を再送 | 重複せず元の結果が返る | 検証済み・期待値どおり |
| Discord連携付きで操作 | DB保存とDiscord反映の結果を分けて確認できる | 検証済み・期待値どおり |
| 最新versionで削除 | WebとDiscord側の結果が期待どおり | 検証済み・期待値どおり |
| 15分以上経過後に読み取り | 更新トークンで接続を継続できる | 検証済み・期待値どおり |
| 接続管理から解除 | 以後の操作が拒否され、通常Webログインは維持される | 検証済み・期待値どおり |

上記の成功だけでは、次の個別確認まで完了した扱いにしない。

| 残手順 | 期待する結果 | 結果 |
| --- | --- | --- |
| URL登録→metadata→Discordログイン→同意の個別確認 | 固定issuer/resource、CIMD、PKCE S256。クライアントIDと未選択のサーバー/操作を表示 | 未検証 |
| 読み取りだけ許可→一覧/詳細→作成を要求、未許可サーバーの参照 | 許可内だけ取得、作成・未許可サーバー参照を拒否 | 未検証 |
| 再接続時の権限選択と操作確認方針 | 書き込みに明示的に同意し、サービス側の操作ごとの確認なし | 未検証 |
| 古いversionで削除 | 競合として拒否 | 未検証 |
| 同じ作成キーで入力を変えて再送 | 内容違いを拒否、重複予定なし | 未検証 |
| 更新トークンの期限切れ・失効 | 再認証を案内 | 未検証 |
| 権限変更・退出・Bot退出→読み取り/書き込み | 読み取りは既存キャッシュ期限内、書き込みは現在権限で拒否 | 未検証 |
| 接続解除後のトークン更新 | 更新を拒否 | 未検証 |
...
| #216 完了条件 | 根拠と行き先 |
| --- | --- |
| 公開範囲・版・仕様 | この文書の検証対象。デスクトップ26.908.40834の8項目は確認済み。実Codex交渉・CLIの実機結果は #220 に残す |
| URL→Discord→同意→PKCE→読み取りPoC | #217のモックDB結合テストと上記の実機8項目。metadata・PKCE等の個別確認は #220 に残す |
| Rustまでの認証・scope・guild・管理API拒否 | #218、api/src/mcp.rsのテスト、web/e2e/mcp-origin.spec.ts。実経路は #220 |
| 同意・解除・期限・権限反映・確認方針 | mcp-auth/read/write.md。更新トークンでの継続利用・解除後の操作拒否は実機確認済み。権限変更・停止等は #220 |
| 入出力・日時・競合・再送・副作用 | #218/#219、mcp-read/write.mdと各テスト。上記8項目は実Discordを含め確認済み、障害・異常系等は #220 |
...
- PostgreSQL 18.6の一時DB `mcp_217_test` を用いたOAuth結合テスト: 14件成功。
  ...
  Discordはモックであり、ステージングの実機検証済みとは扱わない。
